defmodule NameThatSprint.Game do
  ...
  def handle_call({:add_idea, name}, _from, state_data) do
    ...
    case existing_idea do
      ...
      _idea ->
        {:reply, {:error, :duplicate_name}, state_data, @timeout}
    end
  end
  ...
  def handle_call(:declare_winner, _from, state_data) do
    case state_data
         ...
         |> Enum.at(0) do
      nil ->
        {:reply, {:error, :no_ideas}, state_data, @timeout}
  ...
  defp check_if_votes_left({:error, reason, state_data}, _user, _num_votes),
    do: {:error, reason, state_data}

  defp check_if_votes_left({:ok, state_data, idea}, user, nil), do: {:ok, state_data, idea, user}

  defp check_if_votes_left({:ok, state_data, idea}, user, num_votes)
       when num_votes < @max_votes do
    {:ok, state_data, idea, user}
  end

  defp check_if_votes_left({:ok, state_data, _idea}, _user, _num_votes),
    do: {:error, :no_votes_left, state_data}
  ...
  defp update_votes({:error, reason, state_data}, _modifier),
    do: {:reply, {:error, reason}, state_data, @timeout}

  defp update_votes({:ok, state_data, {idea, index}, user}, modifier) do
    idea =
      update_in(idea[:votes], fn votes ->
        case modifier do
